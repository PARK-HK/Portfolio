<!--내용시작 -->
        <div class="sub sub03-1">
            <div class="sub-top" data-aos="fade-down">
                <h2 data-aos="zoom-in">채용가이드</h2>
                <ul data-aos="zoom-in">
                    <li><a href="/main">H</a></li>
                    <li>인재채용</li>
                    <li>채용가이드</li>
                </ul>
            </div>
            <div class="sub-content" data-aos="fade-up">
                <div class="nav">
                    <ul>
                        <li><a href="/sub/s03_1_1">채용전형</a></li>
                        <li class="on"><a href="javascript:;">업무소개</a></li>
                    </ul>
                </div>
                <div class="layout">
                    <div class="tab-btn">
                        <a href="" class="tab">정비</a>
                        <ul class="work-list">
                            <li><a href="/sub/s03_1_2_1">제선</a></li>
                            <li><a href="/sub/s03_1_2_2">제강/연주</a></li>
                            <li><a href="/sub/s03_1_2_3">열연후판/냉연생산</a></li>
                            <li class="on"><a href="javascript:;">정비</a></li>
                            <li><a href="/sub/s03_1_2_5">생산지원</a></li>
                            <li><a href="/sub/s03_1_2_6">사업지원</a></li>
                            <li><a href="/sub/s03_1_2_7">경영지원</a></li>
                            <li><a href="/sub/s03_1_2_8">안전</a></li>
                        </ul>
                    </div>
                    <div class="work-infor">
                        <table>
                            <colgroup>
                                <col width="30%" />
                                <col width="70%" />
                            </colgroup>
                            <thead>
                                <tr>
                                    <th colspan="2" scope="col">기계</th>
                                </tr>
                            </thead>
                            <tbody>
                                <tr>
                                    <th>직무소개</th>
                                    <td>원활한 생산활동을 위해 각 공정/공장별 기계설비를 유지보수하는 한편 설비개선을 위해 설비설계 및 정비하는 업무</td>
                                </tr>
                                <tr>
                                    <th>전공분야</th>
                                    <td>
                                        <ul>
                                            <li>- 기계</li>
                                        </ul>
                                    </td>
                                </tr>
                                <tr>
                                    <th>관련 자격증</th>
                                    <td>
                                        <ul class="w-20">
                                            <li>- 일반기계기사</li>
                                            <li>- 기계설계기사</li>
                                            <li>- 설비보전기사</li>
                                        </ul>
                                        <ul class="w-20">
                                            <li>- 전기기사</li>
                                            <li>- 전기공사기사</li>
                                            <li>- 전자기사</li>
                                        </ul>
                                        <ul class="w-20">
                                            <li>- 정보통신기사</li>
                                        </ul>
                                    </td>
                                </tr>
                            </tbody>
                        </table>
                        <table>
                            <colgroup>
                                <col width="30%" />
                                <col width="70%" />
                            </colgroup>
                            <thead>
                                <tr>
                                    <th colspan="2" scope="col">전기</th>
                                </tr>
                            </thead>
                            <tbody>
                                <tr>
                                    <th>직무소개</th>
                                    <td>각 공정/공장별 전기설비 및 수배전 설비를 유지보수하고 개선하여 생산이 차질없이 진행케 하는 업무</td>
                                </tr>
                                <tr>
                                    <th>전공분야</th>
                                    <td>
                                        <ul>
                                            <li>- 전기/전자</li>
                                        </ul>
                                    </td>
                                </tr>
                                <tr>
                                    <th>관련 자격증</th>
                                    <td>
                                        <ul class="w-20">
                                            <li>- 일반기계기사</li>
                                            <li>- 기계설계기사</li>
                                            <li>- 설비보전기사</li>
                                        </ul>
                                        <ul class="w-20">
                                            <li>- 전기기사</li>
                                            <li>- 전기공사기사</li>
                                            <li>- 전자기사</li>
                                        </ul>
                                        <ul class="w-20">
                                            <li>- 정보통신기사</li>
                                        </ul>
                                    </td>
                                </tr>
                            </tbody>
                        </table>
                        <table>
                            <colgroup>
                                <col width="30%" />
                                <col width="70%" />
                            </colgroup>
                            <thead>
                                <tr>
                                    <th colspan="2" scope="col">설비</th>
                                </tr>
                            </thead>
                            <tbody>
                                <tr>
                                    <th>직무소개</th>
                                    <td>각 공장의 동력, 환경설비, 유틸리티(Utility) 설비 등의 운영 및 유지보수와 설비개선을 하는 업무</td>
                                </tr>
                                <tr>
                                    <th>전공분야</th>
                                    <td>
                                        <ul>
                                            <li>- 기계</li>
                                        </ul>
                                    </td>
                                </tr>
                                <tr>
                                    <th>관련 자격증</th>
                                    <td>
                                        <ul class="w-20">
                                            <li>- 일반기계기사</li>
                                            <li>- 기계설계기사</li>
                                            <li>- 설비보전기사</li>
                                        </ul>
                                        <ul class="w-20">
                                            <li>- 전기기사</li>
                                            <li>- 전기공사기사</li>
                                            <li>- 전자기사</li>
                                        </ul>
                                        <ul class="w-20">
                                            <li>- 정보통신기사</li>
                                        </ul>
                                    </td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
                </div>
            </div>
        </div>
        <!-- 내용끝 -->
		<script>  
            function slideToggle(){
                var t = $(this);
                if(!t.next().hasClass('on')){
                    t.next().addClass('on');
                }else{
                    t.next().removeClass('on');
                }
                return false;
            }
            $('.tab-btn>a').click(slideToggle);
        </script>